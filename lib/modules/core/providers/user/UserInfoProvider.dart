...
// 사용자 정보를 가져오는 공급자
final userDataLoadProvider = FutureProvider<UserDTO>((ref) async {
  final userId = await ref.watch(userIdProvider.future);
  if (userId == null) throw Exception('User not logged in');
  final userDoc =
      await FirebaseFirestore.instance.collection('users').doc(userId).get();
  if (userDoc.exists) {
    return UserDTO.fromMap(userDoc.data()!, userId);
  }
  throw Exception('User not found');
});

final userInfoNotifierProvider =
    StateNotifierProvider<UserNotifier, UserDTO>((ref) {
  final user = ref.watch(userDataLoadProvider);
  return UserNotifier(user.maybeWhen(
      data: (user) => user,
      orElse: () => UserDTO(
          userId: '익명', name: '익명', email: '', age: 123, profileImageUrl: '')));
});

// 유저 정보 업데이트를 위한 노티파이어
class UserNotifier extends StateNotifier<UserDTO> {
  UserNotifier(UserDTO state) : super(state);
  ...
  Future<void> updateUser(
    ...
    try {
      ...
      await FirebaseFirestore.instance
          ...
          .update(updateData);
      state =
          state.copyWith(name: name, age: age, profilePicture: profilePicture);
    } catch (e) {
